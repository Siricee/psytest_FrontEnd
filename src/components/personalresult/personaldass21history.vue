      ...
      getData() {
        ...
        new Promise((resolve, reject) => {
         ...
         for(let data of res){
            ...
            let obj = {'date': datatime, 'depression': parseInt(data.depression), 'anxiety': parseInt(data.anxiety), 'stress':parseInt(data.stress)};
            this.lineChartData.rows.push(obj);
          }
         let lastdata = res[res.length-1];
         let lastobj = {'类别': '您的最近一次测试值', 'anxiety':lastdata.anxiety, 'depression': lastdata.depression, 'stress': lastdata.stress};
...
<style>
  .el-table .warning-depression-row td:nth-child(3),
  .el-table .warning-anxiety-row td:nth-child(4),
  .el-table .warning-stress-row td:nth-child(5) {
    color: red;
  }
</style>
